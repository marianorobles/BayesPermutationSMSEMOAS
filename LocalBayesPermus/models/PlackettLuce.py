...
class PlackettLuceDirichlet(Base):
  ...
  def calculate_top_ranking_probs(self, permus, weights=None):
    ...
    samples = self.sample_posterior(permus, weights)#nuestra muestra(4,1000)
    num_permus, num_algorithms = permus.shape#numero de permutaciones (1000, 4)
    num_posterior_samples = len(samples)
    probs = np.zeros((num_posterior_samples, num_algorithms))#(1000,4)
